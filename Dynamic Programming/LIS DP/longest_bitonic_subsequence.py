class Solution:
    def LongestBitonicSequence(self, nums):
        n = len(nums)
        inc_dp = [1 for i in range(n)] # for a particular index, it will store LIS from 0th to ith index
        inc_rev_dp = [1 for i in range(n)] # for a particular index, it will store LIS from (n-1)th to ith index
