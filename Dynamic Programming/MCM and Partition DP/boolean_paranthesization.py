    ...
    def countWays(self, N, S):
        ...
        def f(start, end, isTrue):  # start, end, what_we_are_searching_for
            ...
            if start == end:
                if (
                    isTrue
                ):  # if we are searching for true then will return 1 if s[start] = 'T'
                    if S[start] == "T":
                        return 1
                    else:
                        return 0
            ...
            for ind in range(start + 1, end, 2):
                lt = f(
                    start, ind - 1, 1
                )  # will return no of ways we can get True if we evaluate only S[start:ind]
                lf = f(start, ind - 1, 0)
                rt = f(ind + 1, end, 1)
                rf = f(ind + 1, end, 0)
        ...
        return f(
            0, N - 1, 1
        )  # we will check from 0th index to (N-1)th index how many ways we can get True(1 signifies that)
