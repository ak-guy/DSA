...
class Solution:
    def get_closest_with_base(self, curr_val, curr_ind, toppingCosts, target):
        if abs(target - curr_val) < abs(target - self.result) or (
            abs(target - curr_val) == abs(target - self.result)
            and self.result > curr_val
        ):
        ...
        self.get_closest_with_base(
            curr_val + toppingCosts[curr_ind], curr_ind + 1, toppingCosts, target
        )
        self.get_closest_with_base(
            curr_val + toppingCosts[curr_ind] * 2, curr_ind + 1, toppingCosts, target
        )
