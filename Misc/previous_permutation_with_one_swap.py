...
class Solution:  # # Leetcode (previous permutation with one swap)
    def prevPermOpt1(self, arr):
        ...
        for i in range(n - 1, bp - 1, -1):
            if (
                arr[bp] > arr[i] and (i > 0 and arr[i] != arr[i - 1])
            ):  # this cond -> (i>0 and arr[i] != arr[i-1]) is because there can exist duplicate, so we will pick last duplicate elem present if we start traversing from back
