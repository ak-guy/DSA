...
class Solution:
    def minHeightShelves(self, books: List[List[int]], shelfWidth: int) -> int:
        ...
        def f(ind, currentSelfMaxHeight, remainingWidth):
            ...
            differentSelf = currentSelfMaxHeight + f(ind+1, currentSelfMaxHeight=books[ind][1], remainingWidth=shelfWidth-books[ind][0])

            # keeping book on same shelf
            sameSelf = 1_000_000_000
            if remainingWidth >= books[ind][0]:
                sameSelf = f(ind+1, currentSelfMaxHeight=max(currentSelfMaxHeight, books[ind][1]), remainingWidth=remainingWidth-books[ind][0])
