...
class Solution:
        ...
        while pq:
            ...
            for it in gr[node]:
                ...
                if distanceTravelled + weight < resArray[neighbourNode]:
                    ...
                    if (
                        not reachableCity[neighbourNode]
                        and resArray[neighbourNode] <= distanceThreshold
                    ):
                        res += 1
    ...
    def findTheCity(
        ...
        gr = {}
        for edge in edges:
            if not gr.get(edge[0]):
                gr[edge[0]] = [[edge[1], edge[2]]]
            else:
                gr[edge[0]].append([edge[1], edge[2]])

            if not gr.get(edge[1]):
                gr[edge[1]] = [[edge[0], edge[2]]]
            else:
        ...
        for city in range(n):
            citiesReachable = (
                self.getMinimumCitiesReachable(city, gr, distanceThreshold, n)
                if city in gr
                else 0
            )
