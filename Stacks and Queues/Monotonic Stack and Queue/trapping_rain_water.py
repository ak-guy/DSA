...
class Solution:  # TC -> O(N) and SC -> O(N)
    def trappingWater(self, arr, n):
        ...
        for i in range(
            n
        ):  # at any index 'i', the amount of water it can store is => min(max_building_height to the left, max_building_height to the right) - ith_building_height
    ...
    def trappingWater(self, arr, n):
        ...
        while l <= r:
            if arr[l] <= arr[r]:
                ...
                main_res += (
                    left_max - arr[l]
                )  # we add like this in res because we are sure that the max_height of builing on the left will be atleast of length arr[l]
                l += 1
            else:
                right_max = max(right_max, arr[r])
                main_res += (
                    right_max - arr[r]
                )  # we add like this in res because we are sure that the max_height of builing on the right will be greater than arr[r]
                r -= 1
