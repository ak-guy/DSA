class Solution:
    ...
    def InfixtoPostfix(self, exp):
        ...
        for char in exp:
            if (
                char.isalnum()
            ):  # if char is alphabet or number we can directly add in res
                res += char
            else:
                if char == "(":  # if char is '(' then append it in stack
                    ...
                    char == ")"
                ):  # if char is ')' then pop from stack till and including '('
                    while st[-1] != "(":
                        res += st.pop()
                    st.pop()  # to remove '('
                else:  # if char is some operator
                    while (
                        st and precedence_mapping[char] <= precedence_mapping[st[-1]]
                    ):  # we need to pop all operator which have higher precedence
                        res += st.pop()
                    st.append(
                        char
                    )  # adding the operator after poping all higher precedence operator
