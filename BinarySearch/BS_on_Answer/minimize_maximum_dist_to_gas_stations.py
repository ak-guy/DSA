...
class Solution:
    def findSmallestMaxDist(self, stations, K):
        n = len(stations)
        pq = []
        no_gas_station_bw_interval = [
            0 for i in range(n - 1)
        ]  # this we will use to maintain the positions where we had built gas station

        for i in range(n - 1):  # TC -> nlogn && SC -> n-1
            heapq.heappush(
                pq, (-1 * (stations[i + 1] - stations[i]), i)
            )  # taking negative to convert it to maxheap
...
class Solution:
    def getRequiredGasStations(self, stations, min_dist_needed_to_be_satisfied, n):
        used_gas_stations = 0
        for i in range(n - 1):
            required_gas_station = (
                stations[i + 1] - stations[i]
            ) / min_dist_needed_to_be_satisfied
            if (
                stations[i + 1] - stations[i]
                == required_gas_station * min_dist_needed_to_be_satisfied
            ):
