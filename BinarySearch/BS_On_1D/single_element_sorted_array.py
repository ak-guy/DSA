...
def singleNonDuplicate(arr):
    ...
    while l <= r:
        ...
        if (
            arr[m] != arr[m - 1] and arr[m] != arr[m + 1]
        ):  # case when middle element is the single element in array
            return arr[m]
        ...
        if arr[m] == arr[m + 1]:
            if (m - l) % 2:
                ...
                l = (
                    m + 2
                )  # need to exclude the m+1 index as m and m+1 index element is same
        else:
            if not (m - l) % 2:
                r = (
                    m - 2
