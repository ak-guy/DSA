def solve(arr1, arr2, n, m, k):
    ...
    low = max(
        0, k - m
    )  # this low will be greater than 0 in case if we have to take something from the first array. Ex- n=3, m=6, k=7 in this case we need atleast one value from first array
